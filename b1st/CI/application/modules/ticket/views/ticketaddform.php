	 ...
	 <?php
       ...
       <!--another_extra_area-->
         ...
         <div class="product_box" id="side_area">
          <div class="headding_bl">
           <p><span><i class="fa fa-th-list"></i></span><?php echo $this->lang->line('Add Ticket');?></p>
          </div>
	  ...
	  <form action="<?= TICKET_PLUGIN_URL;?>CI/index.php/ticket/insert" method="post" enctype="multipart/form-data">
		...
		<div class="form_holder">
		  <label><span><?php echo $this->lang->line('Ticket No');?></span></label>
		  <input type="text" name="ticket_no" placeholder="<?php echo $this->lang->line('Enter');?> <?php echo $this->lang->line('Ticket No');?>" value="<?= B1st_create_guid(); ?>" readonly="">
		</div>
		
		<div class="form_holder">
		  <label><span><?php echo $this->lang->line('Subject');?></span></label>
		  <input type="text" name="subject" placeholder="<?php echo $this->lang->line('Enter');?> <?php echo $this->lang->line('Subject');?>" value="<?php if(isset($subject)) { echo $subject; } else { echo set_value('subject'); } ?>">
		</div>
		
		<div class="form_holder">
		  <label><span><?php echo $this->lang->line('Customer Email');?></span></label>
		  <input type="text" name="customer" placeholder="<?php echo $this->lang->line('Enter');?> <?php echo $this->lang->line('Customer Email');?>" value="<?php if(isset($email)) { echo $email; } else { echo set_value('customer'); } ?>">
		</div>
               </div>
	       ...
	       <div class="form_holder">
			...
			if(!empty($companydet))
			{
			  ...
			  {
			    ?>
			    <option <?php if($companydetlist->id==set_value('company_id')) { ?> selected="true" <?php } ?> value="<?php echo $companydetlist->id;?>"><?php echo stripslashes($companydetlist->company_name);?></option>
			    <?php
			  }
			}
			?>
	       ...
	       ?>
			...
			if(!empty($departmentdet))
			{
			  ...
			  {
			    ?>
			    <option <?php if($departmentdetlist->id==set_value('department_id')) { ?> selected="true" <?php } ?> value="<?php echo $departmentdetlist->id;?>"><?php echo stripslashes($departmentdetlist->department_name);?></option>
			    <?php
			  }
			}
			?>
	       ...
	       <div class="form_holder">
			...
			if(!empty($productdet))
			{
			  ...
			  {
			    ?>
			    <option <?php if($productdetlist->id==set_value('product_id')) { ?> selected="true" <?php } ?> value="<?php echo $productdetlist->id;?>"><?php echo stripslashes($productdetlist->product_name);?></option>
			    <?php
			  }
			}
			?>
      ...
      if(!empty($prioritydet))
      {
        ...
        {
          ?>
          <option <?php if($prioritydetlist->id==set_value('priorty')) { ?> selected="true" <?php } ?> value="<?php echo $prioritydetlist->id;?>"><?php echo stripslashes($prioritydetlist->priority_name);?></option>
          <?php
        }
      }
      ?>
               ...
               <div class="form_holder">
                 ...
                 <textarea name="query" cols="" rows="" placeholder="<?php echo $this->lang->line('Enter');?> <?php echo $this->lang->line('Query');?>"><?php if(isset($query)) { echo $query; } else { echo set_value('query'); } ?></textarea>
               </div>
		...
		<?php
	       	if(!empty($attachments))
	       	{
				...
				{
				?>	       		
				    <li id="delfil<?php echo $attachment->id; ?>">
				    <?php echo $attachment->filename; ?> <span onclick="delfile('<?php echo $attachment->id; ?>','<?php echo $attachment->filename; ?>')"><i class="fa fa-times-circle"></i></span>
				    <input type="hidden" value="<?php echo $attachment->filename; ?>" readonly="true" name="files[]">
				    </li>
				<?php
				}
				?>
	        ...
	        ?>
               ...
               <?php if(!empty($email_no)){ ?>
		 <input type="hidden" name="email_no" value="<?php echo $email_no ?>" />
               <?php } ?>

                              <?php if(!empty($tweet_id)){ ?>
		 <input type="hidden" name="tweet_id" value="<?php echo $tweet_id ?>" />
	      ...
	      <form class="uploadfiles" id="myForm" action="<?= TICKET_PLUGIN_URL;?>CI/index.php/ticket/insert" method="post" enctype="multipart/form-data">
