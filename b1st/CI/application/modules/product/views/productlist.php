...
?>
       ...
       <div class="page_title">
         <div class="headline">
          <h1><span><i class="fa fa-gift"></i></span><?php echo $this->lang->line('Product');?></h1>
         </div>
         <div class="right_panel">
           ...
           <a href="<?= TICKET_PLUGIN_URL;?>CI/index.php/product/add" class="bttn pi-btn btn-no-border"><span class="edit_all"><i class="fa fa-plus"></i></span><?php echo $this->lang->line('Add Product');?></a>
	...
	<?php $this->load->view('common/ticketpanel');?>
         ...
         <div class="product_box" id="side_area">
          <div class="headding_bl">
           <p><span><i class="fa fa-th-list"></i></span><?php echo $this->lang->line('Manage Products');?></p>
            <form id="pageNo" action="<?= TICKET_PLUGIN_URL;?>CI/index.php/product/index/<?= $this->uri->segment(3) ?>" method="post">
       ...
       <tbody>
		  ...
		  if(!empty($productdet))
		  {
			...
			<tr <?php if($i%2==0) { ?> class="white" <?php } else  { ?> class="gray" <?php } ?>>
			     ...
			     <td><?php echo stripslashes($productdetlist->product_name);?></td>
			     <!--<td><?php //echo stripslashes($productdetlist->subject);?></td>-->
			     <!--<td><?php //echo stripslashes($productdetlist->department);?></td>-->
			     <!--<td><?php //echo stripslashes($productdetlist->customer);?></td>-->
			     <td <?php if($productdetlist->status==1) { ?> class="act" <?php } if($productdetlist->status==0) { ?> class="inact" <?php } ?>>
				<?php
				if($productdetlist->status==1)
				{
				     ?>
				     <p><a style="color:#fff;" href="<?= TICKET_PLUGIN_URL;?>CI/index.php/product/statuschange/<?php echo $productdetlist->id;?>"><?php echo $this->lang->line('Active');?></a></p>
				     <?php
				}
				if($productdetlist->status==0)
				{
				     ?>
				     <p><a style="color:#fff;" href="<?= TICKET_PLUGIN_URL;?>CI/index.php/product/statuschange/<?php echo $productdetlist->id;?>"><?php echo $this->lang->line('Inactive');?></a></p>
				     <?php
				}
				?>
			     </td>
			     <!--<td <?php //if($productdetlist->priorty=="High") { ?> class="highp" <?php //} if($productdetlist->priorty=="Medium") { ?> class="medp" <?php //} if($productdetlist->priorty=="Low") { ?> class="lowp" <?php //} ?>>-->
				     <!--<p><?php //echo $productdetlist->priorty;?></p>	-->
			     <!--</td>-->

			     <td>	
				     <button type="submit" class="edit_button bitn nuc_bl has-tip" title="<?php echo $this->lang->line('Edit');?>" onclick="window.location.href='<?= TICKET_PLUGIN_URL;?>CI/index.php/product/edit/<?php echo $productdetlist->id;?>'"><i class="fa fa-pencil"></i></button>
<!--              <button type="submit" class="edit_button bitn nuc_bl"><i class="fa fa-eye"></i>View</button> -->
               ...
               if($settings['type'] == 1)
               {
		$conftxt=$this->lang->line('Are you sure you want to delete this product Permanently');
                $txt = "getConfirmation('".$conftxt.".','".TICKET_PLUGIN_URL."CI/index.php/product/deleteproduct/". $productdetlist->id."')";
               }
               else
               {
                $txt = "window.location.href='".TICKET_PLUGIN_URL."CI/index.php/product/deleteproduct/".$productdetlist->id."'";
               }
            ?>
