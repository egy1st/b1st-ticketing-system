		  ...
		  if(!empty($prioritydet))
		  {
           ...
           <p style="background-color:<?= $prioritydetlist->priority_color ?>"><a style="color:#fff;" href="javascript:void(0);"><?= $prioritydetlist->priority_color ?></a></p>   
			   ...
			   <!--   <td><?php echo date("dS M, Y",strtotime($prioritydetlist->create_date));?></td> -->
			     <td>	
				     <button type="submit" class="edit_button bitn nuc_bl has-tip" title="<?php echo $this->lang->line('Edit');?>" onclick="window.location.href='<?= TICKET_PLUGIN_URL;?>CI/index.php/ticketpriority/edit/<?php echo $prioritydetlist->id;?>'"><i class="fa fa-pencil"></i></button>
